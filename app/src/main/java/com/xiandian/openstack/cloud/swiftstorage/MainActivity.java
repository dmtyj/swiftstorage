...
//        initTensorFlowAndLoadModel();
    ...
    public boolean onCreateOptionsMenu(Menu menu) {//(4)创建操作栏
        ...
        if(currentFragment==garbageFragment){
            ...
            menu.findItem(R.id.action_take_photo_upload).setVisible(false);
            menu.findItem(R.id.action_get_location_photo_upload).setVisible(false);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {//(5)工具栏选择操作
        ...
        //拍照识别
        else if (id == R.id.action_take_photo_upload) {
            this.currentFragment.takePhoto_upload();
        }
        //获取本地照片识别后上传
        else if (id == R.id.action_get_location_photo_upload) {
            this.currentFragment.getLocationPhotoUpload();
        }
        //重定名
    ...
    public boolean onNavigationItemSelected(MenuItem item) {//导航选择操作
        ...
        //文档
            ...
            //调用文档的CategoryFragment并传入参数MIME_DOC
            if (docCategoryFragment == null) {
                docCategoryFragment = new CategoryFragment();
                Bundle bundle = new Bundle();
                bundle.putStringArray(Constants.CATEGORY_TYPE, Constants.MIME_DOC);
                docCategoryFragment.setArguments(bundle);
            }
            fragmentManager.beginTransaction().replace(R.id.container, docCategoryFragment).commit();
            currentFragment = docCategoryFragment;

            //改变工具栏
            setToolbarTitles(getString(R.string.menu_doc), "");
        }
        //图片
        ...
        //音频
            ...
            //调用视频的CategoryFragment并传入参数MIME_VIDEO
            if (audioCategoryFragment == null) {
                audioCategoryFragment = new CategoryFragment();
                Bundle bundle = new Bundle();
                bundle.putStringArray(Constants.CATEGORY_TYPE, Constants.MIME_AUDIO);
                audioCategoryFragment.setArguments(bundle);
            }
            fragmentManager.beginTransaction().replace(R.id.container, audioCategoryFragment).commit();
            currentFragment = audioCategoryFragment;
            //改变工具栏
            setToolbarTitles(getString(R.string.menu_music), "");
        }
        //视频
        else if (id == R.id.nav_video) {
            //调用视频的CategoryFragment并传入参数MIME_VIDEO
            if (videoCategoryFragment == null) {
                videoCategoryFragment = new VideoFragment();
                Bundle bundle = new Bundle();
                bundle.putStringArray(Constants.CATEGORY_TYPE, Constants.MIME_VIDEO);
                videoCategoryFragment.setArguments(bundle);
            }
            fragmentManager.beginTransaction().replace(R.id.container, videoCategoryFragment).commit();
            currentFragment = videoCategoryFragment;

            setToolbarTitles(getString(R.string.menu_video), "");
        }
        //回收站
            ...
            fragmentManager.beginTransaction().replace(R.id.container, garbageFragment).commit();
            currentFragment = garbageFragment;
            setToolbarTitles(getString(R.string.menu_recycle), "");
        }
        //点击后关闭导航面板
    ...
    public List<Classifier.Recognition> getTakePhoto_Result(Bitmap bitmap){
//        initTensorFlowAndLoadModel();
        List<Classifier.Recognition> result = null;
        //将当前的图片按一定比例创建一张新的图
